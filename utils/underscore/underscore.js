...
!function () {
    ...
    h.viewimg = function (e, img_arr) {
        var src = "";
        if (this.isObject(e)) {
            src = e.currentTarget.dataset.src; // 当前显示图片的http链接
        } else {
            src = e;
        }
        wx.previewImage({
    ...
    h.wxjsonDecode = function (arr, field,default_data=[]) {
        if (this.isArray(arr)) {
            for (var i = 0; i < arr.length; i++) {
                if (arr[i][field] != undefined && arr[i][field] != '') {
                    arr[i][field] = JSON.parse(arr[i][field]);
                }else{
                    arr[i][field] = default_data;
                }
            }
            return arr;
        } else {
            if (arr[field] != undefined && arr[field] != '') {
                arr[field] = JSON.parse(arr[field]);
            }else{
                arr[i][field] = default_data;
            }
            return arr;
    ...
    h.wxfindIndex = function (arr, obj) {
        var index = -1;
        for (var i = 0; i < arr.length; i++) {
            var row = arr[i];
            if (row == obj || (this.isObject(obj) && this.isMatch(row, obj))) {
                return i;
            } else {
                continue;
            }
    ...
    h.wxpluck = function (list, propertyName) { //萃取 对象数组 中某个对象属性 组成一个数组
        return this.pluck(list, propertyName);
    };

    h.wxfindList = function (arr, obj) {
        var list = this.where(arr, obj);
        return list;
    };

    h.showModal = function (confirm_callBack, content, title, err_callBack) {
        if (!content) {
            content = "";
        }
        if (!title) {
        ...
        wx.showModal({
            ...
            success: function (res) {
                if (res.confirm) {
                    ...
                    typeof err_callBack == 'function' && err_callBack();
                }
            }
        });
    };

    h.show_loading = function () {
        wx.showLoading({
            title: '加载中...',
        });
    };

    h.makeCall = function (e) {//电话
        var number;
        if (this.isObject(e)) {
    ...
    h.hide_loading = function () {
        wx.hideLoading();
    };
    h.tips = function (title, images, during) {
        if (!during) {
            during = 2000;
        }
        if (images == undefined) {
            images = '/resources/icon_tip.png';
        }
        wx.showToast({
            title: title,
            image: "",
    ...
    h.success_tips = function (txt, during) {
        if (!during) {
            during = 1500;
        }
        wx.showToast({
    ...
    h.setNavigationBarTitle = function (title) {
        wx.setNavigationBarTitle({
            title: title,
        });
    };

    h.is_route_referer = function (refer) {
        var pages = getCurrentPages();
        if (pages.length > 1 && pages[pages.length - 2] != undefined && pages[pages.length - 2].route.indexOf(refer) != -1) {
            return true;
        } else {
            return false;
        }
    ...
    h.input_value = function (e) {
        var val = e.detail.value;
        return val;
    };

    h.searchInput = function (e) {
        let input_text = e.detail.value;
        if (this.has(input_text, "search")) {
    ...
    h.wxpreview = function ($url) {
        ...
        h.combineURLs = function (baseURL, relativeURL) { //合并路径 baseurl 与相对凌
