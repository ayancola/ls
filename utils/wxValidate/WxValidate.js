...
var ruleObj = require("validate_rule");

class WxValidate {
    constructor(scene) {
        var rules = {}, messages = {};
        var all_rules = ruleObj.rules;
        var all_messages = ruleObj.messages;
        if (us.has(ruleObj.scene, scene)) {
            var field_arr = ruleObj.scene[scene];
            for (var i = 0; i < field_arr.length; i++) {
                if (!us.has(all_rules, field_arr[i]) || !us.has(all_messages, field_arr[i])) {
                    throw new Error(field_arr[i] + '验证字段没定义');
                } else {
                    rules[field_arr[i]] = all_rules[field_arr[i]];
                    messages[field_arr[i]] = all_messages[field_arr[i]];
                }
            }
        } else if (us.isArray(scene)) {
            var field_arr = scene;
            for (var i = 0; i < field_arr.length; i++) {
                if (!us.has(all_rules, field_arr[i]) || !us.has(all_messages, field_arr[i])) {
                    throw new Error(field_arr[i] + '验证字段没定义');
